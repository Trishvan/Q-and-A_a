...
import { indexDocument, searchRelevant } from "./vectorStore.js";
...
// index PDF
console.log("Indexing PDF into vector DB...");
await indexDocument(pdfText);
console.log("Indexing complete. Total chunks:", vectorDB.length);

// retrieve chunks relevant to the question
console.log("Running vector search...");
const topChunks = await searchRelevant(question, 5);

const context = topChunks.map(c => c.text).join("\n\n");

// this replaces the full pdfText in Llama call:
console.log("Calling Llama with vector-retrieved context...");
answer = await askWithLlama(context, question);
